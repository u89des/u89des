...
function WorkOrderRoom({ order, data, access, refresh, onToast }) {
  ...
  return <section className="panel work-order-room creative-work-room"><header className="work-order-room-head"><div><span>{order.reference}، {project?.name || "مشروع"}</span><h2>{order.title}</h2><small>{modeLabels[executionMode]}</small></div><LiveStatus value={order.status} /></header>{!order.dispatched_at && !["client_review", "completed"].includes(order.status) && <div className="work-order-private-note creative-owner-note"><LockKey size={19} /><p><strong>هذه مساحة عبد الوهاب الإبداعية.</strong> لا يرى المتعاون البحث أو الفكرة أو الملفات قبل تثبيت الاتجاه وتفويض نطاق إنتاجي محدد.</p></div>}
    ...
    {editable && <div className="creative-settings-row"><label>نمط التنفيذ<select value={executionMode} onChange={(event) => setExecutionMode(event.target.value)}><option value="owner_led">عبد الوهاب يقود وينفذ</option><option value="delegated">عبد الوهاب يقود ويفوض الإنتاج</option><option value="collaborative">قيادة عبد الوهاب مع تنفيذ مشترك</option></select></label><label>الأولوية<select value={priority} onChange={(event) => setPriority(event.target.value)}><option value="low">منخفضة</option><option value="normal">عادية</option><option value="high">عالية</option><option value="urgent">عاجلة</option></select></label><label>الموعد الداخلي<input type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} /></label></div>}
