...
export default function PortfolioDesk({ access }) {
  ...
  return <section className="panel page-stack"><div className="panel-heading"><div><h2>معرض الأعمال: الإعداد والمراجعة</h2><p>ارفع الصور من جهازك. الحفظ والإرسال للمراجعة لا ينشران العمل للزوار.</p></div><button className="button primary" disabled={busy || !data} onClick={create}>إضافة مسودة عمل</button></div>
    ...
    {project && <div className="live-task-form"><fieldset disabled={busy || locked} style={{ border: 0, padding: 0 }}>
      ...
      <label>رفع الشعار<input type="file" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" onChange={(event) => { upload(event.target.files, true); event.target.value = ""; }} /></label>{imageUrl(project.cover) && <img src={imageUrl(project.cover)} alt="معاينة الشعار" style={{ width: 180, height: 150, objectFit: "contain" }} />}
      <label>رفع صور المشروع<input type="file" multiple accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" onChange={(event) => { upload(event.target.files, false); event.target.value = ""; }} /></label><small>JPG، PNG، WebP. حتى 8 ميجابايت للصورة، و20 صورة للمشروع.</small>
