...
export default function PortfolioDesk({ access }) {
  ...
  const kinds = { brand: "علامة تجارية", logo: "شعار", campaign: "حملة وتصاميم" };
  ...
  return <section className="panel page-stack"><div className="panel-heading"><div><h2>معرض الأعمال: الإعداد والمراجعة</h2><p>ارفع الصور من جهازك. الحفظ والإرسال للمراجعة لا ينشران العمل للزوار.</p></div><button className="button primary" disabled={busy || !data} onClick={create}>إضافة مسودة عمل</button></div>
    ...
    {project && <div className="live-task-form"><fieldset disabled={busy || locked} style={{ border: 0, padding: 0 }}>
      ...
      <p>{project.kind === "logo" ? "يظهر في شبكة الشعارات بالأسود، دون صفحة تفاصيل. ارفع صورة كاملة بخلفية شفافة أو بيضاء دون قص." : project.kind === "campaign" ? "تظهر صورة الغلاف والصور الإضافية في قسم الحملات. يمكنك ترتيب الصور أو إزالتها قبل النشر." : "يظهر ضمن بطاقات العلامات مع صفحة تفاصيل المشروع."}</p>
      ...
      <label>{project.kind === "campaign" ? "رفع غلاف الحملة" : "رفع الشعار"}<input type="file" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" onChange={(event) => { upload(event.target.files, true); event.target.value = ""; }} /></label>{imageUrl(project.cover) && <img src={imageUrl(project.cover)} alt="معاينة الغلاف" style={{ width: 180, height: 150, objectFit: "contain" }} />}
